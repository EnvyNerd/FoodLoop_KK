...
import { View, Text, ScrollView, TextInput, TouchableOpacity, StyleSheet, Alert, KeyboardAvoidingView, Platform, Image } from 'react-native';
...
export default function AddProductScreen({ navigation }) {
  ...
  function showImageOptions() {
    Alert.alert('Add Photo', 'Choose how to add a photo for your bag', [
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      await addProduct({
        ...
        tags: tagList, image,
        vendorId: user?.email || 'vendor-001', vendor: 'Sunrise Bakery',
      });
      Alert.alert('Success', 'Your bag has been listed!', [{ text: 'OK', onPress: () => navigation.goBack() }]);
    } catch (e) {
  ...
  return (
    <SafeAreaView style={s.container}>
      <View style={s.topBar}>
        ...
        <Text style={s.heading}>Add New Bag</Text>
        <View style={{ width: 50 }} />
      </View>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={s.flex}>
        <ScrollView contentContainerStyle={s.scrollContent} keyboardShouldPersistTaps="handled">
          ...
          {image && (
            <TouchableOpacity onPress={showImageOptions}><Text style={s.changePhotoText}>Change photo</Text></TouchableOpacity>
          )}
          ...
          <View style={s.chipRow}>
            {HALAL_OPTIONS.map(h => (
              <TouchableOpacity key={h} style={[s.chip, halal === h && s.chipActive]} onPress={() => setHalal(h)}>
                <Text style={[s.chipText, halal === h && s.chipTextActive]}>
                  {h === 'halal' ? '\u2713 Halal' : h === 'mixed' ? 'Mixed' : 'Non-halal'}
                </Text>
              </TouchableOpacity>
            ))}
          </View>
          ...
          <View style={s.row}>
            <View style={s.half}>
              <Text style={s.label}>Current Price (RM)</Text>
              <TextInput style={s.input} placeholder="6" placeholderTextColor={Colors.textTertiary} value={priceNow} onChangeText={setPriceNow} keyboardType="decimal-pad" />
            </View>
            <View style={s.half}>
              <Text style={s.label}>Original Price (RM)</Text>
              <TextInput style={s.input} placeholder="18" placeholderTextColor={Colors.textTertiary} value={priceOriginal} onChangeText={setPriceOriginal} keyboardType="decimal-pad" />
            </View>
          </View>

          <View style={s.row}>
            <View style={s.half}>
              <Text style={s.label}>Pickup Start</Text>
              <TextInput style={s.input} placeholder="19:30" placeholderTextColor={Colors.textTertiary} value={pickupStart} onChangeText={setPickupStart} />
            </View>
            <View style={s.half}>
              <Text style={s.label}>Pickup End</Text>
              <TextInput style={s.input} placeholder="20:30" placeholderTextColor={Colors.textTertiary} value={pickupEnd} onChangeText={setPickupEnd} />
            </View>
          </View>
          ...
          <TouchableOpacity style={[s.submitBtn, isSubmitting && s.submitBtnDisabled]} onPress={handleSubmit} disabled={isSubmitting}>
            <Text style={s.submitBtnText}>{isSubmitting ? 'Listing...' : '+ List this bag'}</Text>
          </TouchableOpacity>
